...
def user_filter_reason(user: 'AbstractBaseUser'):
    if user.get_username() == 'bar':
        return 'some reason'
    return None
def user_filter_no_reason(user: 'AbstractBaseUser'):
    return ''
def user_filter_false(user: 'AbstractBaseUser'):
    return False
def user_filter_exception(user: 'AbstractBaseUser'):
    if user.get_username() == 'bar':
        raise ValidationError(['first good reason', "anyway, I don't like {0}".format(user.get_username())])
    return None

# exchange_filter function set
def exch_filter_reason(sender: 'AbstractBaseUser', recipient: 'AbstractBaseUser', recipients_list: 'list[AbstractBaseUser] | None'):
    if recipient.get_username() == 'bar':
        return 'some reason'
    return None
def exch_filter_no_reason(sender: 'AbstractBaseUser', recipient: 'AbstractBaseUser', recipients_list: 'list[AbstractBaseUser] | None'):
    return ''
def exch_filter_false(sender: 'AbstractBaseUser', recipient: 'AbstractBaseUser', recipients_list: 'list[AbstractBaseUser] | None'):
    return False
def exch_filter_exception(sender: 'AbstractBaseUser', recipient: 'AbstractBaseUser', recipients_list: 'list[AbstractBaseUser] | None'):
    if recipient.get_username() == 'bar':
        raise ValidationError(['first good reason', "anyway, I don't like {0}".format(recipient.get_username())])
    return None

# auto-moderation function set
def moderate_as_51(message: 'Message'):
    return 51
def moderate_as_48(message: 'Message'):
    return (48, "some reason")
moderate_as_48.default_reason = 'some default reason'  # type: ignore

# quote formatters
def format_subject(subject: str):
    return "Re_ " + subject
def format_body(sender: 'AbstractBaseUser | str', body: str):
