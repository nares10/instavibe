...
if TYPE_CHECKING:
    ...
    from .models import Message, SiteAlias
...
if notifier_app_id:
    try:  # priority to an app label
        ...
        # cast() while https://github.com/sbdchd/django-types/issues/247
        notification = cast('ModuleType | None', notifier_app_config.models_module)  # "None if the application doesn’t contain a models module"
...
if name and name in settings.INSTALLED_APPS:
    ...
    if name == 'mailer':  # the app didn't adjust to the signature change in Django 1.7 (last check: v1.2.2)
        ...
        def send_mail(subject: str, message: str, from_email: str | None, recipient_list: 'Sequence[str] | None', **kwargs: Any) -> int:
...
def format_body(sender: 'AbstractBaseUser | str', body: str, indent: str = _("> "), width: int = WRAP_WIDTH):
    """
...
@sensitive_variables('subject', 'html_message', 'message')
def email(subject_template: str, message_template_name: str, recipient_list: 'Sequence[str]', object: 'Message', action: str, site: 'SiteAlias'):
...
def email_visitor(object: 'Message', action: str, site: 'SiteAlias'):
    """Email a visitor."""
    email('postman/email_visitor_subject.txt', 'postman/email_visitor', [object.email], object, action, site)


def _get_notification_approval(user: 'AbstractUser', action: str, site: 'SiteAlias'):
    """
...
def notify_user(object: 'Message', action: str, site: 'SiteAlias'):
    """Notify a user."""
    if action == 'rejection':
        user = cast('AbstractUser', object.sender)  # discard unexpected None
        label = 'postman_rejection'
    elif action == 'acceptance':
        user = cast('AbstractUser', object.recipient)  # discard unexpected None
