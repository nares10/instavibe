...
from typing import Any, TYPE_CHECKING, Callable, Literal, Sequence, cast
...
if TYPE_CHECKING:
    ...
    from .models import AutoModeratorsAlias, SiteAlias

    class _TypedUser(AbstractUser):
        sent_messages: Manager[Message]
        received_messages: Manager[Message]
        moderated_messages: Manager[Message]
        notification_approval: Callable[[str, Any], bool | str | None]
...
class GenericTest(TestCase):
    ...
    def test_version(self):
        self.assertEqual(sys.modules['postman'].__version__, "4.5.1")
...
class TransactionViewTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.user1 = cast('UserManager[AbstractBaseUser]', get_user_model().objects).create_user('foo', 'foo@domain.com', 'pass')
        self.user2 = cast('UserManager[AbstractBaseUser]', get_user_model().objects).create_user('bar', 'bar@domain.com', 'pass')
        for a in (
...
class BaseTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user1 = cast('UserManager[_TypedUser]', get_user_model().objects).create_user('foo', 'foo@domain.com', 'pass')
        cls.user2 = cast('UserManager[_TypedUser]', get_user_model().objects).create_user('bar', 'bar@domain.com', 'pass')
        cls.user3 = cast('UserManager[AbstractUser]', get_user_model().objects).create_user('baz', 'baz@domain.com', 'pass')
    ...
    def reload_modules(cls):
        ...
        try:
            ...
            pass
        # django-stubs: @cached_property def urlconf_module(self) -> ModuleType | None | Sequence[_AnyURL]: ...
        # django-types: urlconf_module: list[tuple[str, Callable[..., Any]]] | None
        # https://github.com/sbdchd/django-types/issues/248
        reload(cast('ModuleType', get_resolver(cast( str | None, get_urlconf())).urlconf_module))
    ...
    def _assertQuerySetEqual(self, *args: Any, **kwargs: Any):
        if VERSION < (4, 2):
    ...
    def check_now(self, dt: datetime):
        "Check that a date is now. Well... almost."
        delta = dt - now()
        seconds = delta.days * (24*60*60) + delta.seconds
        self.assertTrue(-2 <= seconds <= 1)  # -1 is not enough for Mysql

    def check_status(self, m: 'Message', status: str = STATUS_PENDING, is_new: bool = True, is_replied: bool = False,
        parent: 'Message | None' = None, thread: 'Message | None' = None,
        moderation_date: datetime | bool = False, moderation_by: 'AbstractBaseUser | None' = None, moderation_reason: str = '',
        sender_archived: bool = False, recipient_archived: bool = False,
        sender_deleted_at: datetime | bool = False, recipient_deleted_at: datetime | bool = False
    ):
    ...
    @classmethod
    def create_accepted(cls, *args: Any, **kwargs: Any):
...
class ViewTest(BaseTest):
    ...
    def _assertFormError(self, response: 'HttpResponse', form_name: str, *args: Any):
        if VERSION < (4, 1):
    ...
    def check_redirection_to_login(self, response: 'HttpResponse', url: str):
    ...
    def check_retrieved_msgs(self, url: str, pks: list[int]):
        def pk(x: 'Message'): return x.pk
        response = self.client.get(url)
    ...
    def check_template(self, action: str, args: Sequence[int]):
    ...
    def test_write_recipient(self):
        ...
        cast('UserManager[AbstractBaseUser]', get_user_model().objects).create_user("Le Créac'h", 'foobar@domain.com', 'pass')  # even: space, accentued, quotes
    ...
    def check_init_by_query_string(self, action: str, args: Sequence[int] = []):
    ...
    def check_message(self, m: 'Message', is_anonymous: bool = False, subject: str = 's', body: str = 'b', recipient_username: str = 'bar'):
        ...
        self.assertEqual(cast('AbstractUser', m.recipient).get_username(), recipient_username)
        if is_anonymous:
            self.check_status(m, sender_deleted_at=True)
        self.assertEqual(len(mail.outbox), 0)

    def check_contrib_messages(self, response: 'HttpResponse', text: str):
    ...
    def check_write_post(self, extra: dict[str, str] = {}, is_anonymous: bool = False):
    ...
    def check_404(self, view_name: str, pk: int):
        "Return is a 404 page."
        url = reverse(view_name, args=[pk])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)

    def check_reply_404(self, pk: int):
    ...
    def check_view_404(self, pk: int):
    ...
    def check_view_conversation_404(self, thread_id: int):
    ...
    def test_view_conversation_id(self):
        ...
        m1.replied_at = m2.sent_at; m1.save()
        self.check_view_conversation_404(cast(int, m1.thread_id))
    ...
    def check_update(self, view_name: str, success_msg: str, field_bit: str, pk: int, field_value: bool = False):
        ...
        self.check_status(Message.objects.get(pk=pk),   status=STATUS_ACCEPTED, **{sender_kw: cast(Any, field_value)})
        self.check_status(Message.objects.get(pk=pk+1), status=STATUS_ACCEPTED, **{recipient_kw: cast(Any, field_value)})
        self.check_status(Message.objects.get(pk=pk+2), status=STATUS_ACCEPTED, **{sender_kw: cast(Any, field_value)})
    ...
    def check_update_conversation(self, view_name: str, root_msg: 'Message', field_bit: str, field_value: bool = False):
        ...
        sender_kw = 'sender_{0}'.format(field_bit)
        recipient_kw = 'recipient_{0}'.format(field_bit)
        self.check_status(Message.objects.get(pk=pk), status=STATUS_ACCEPTED, is_new=False, is_replied=True, thread=root_msg, **{sender_kw: cast(Any, field_value)})
        self.check_status(Message.objects.get(pk=pk+1), status=STATUS_ACCEPTED, parent=root_msg, thread=root_msg, **{recipient_kw: cast(Any, field_value)})
        # missing payload
    ...
    def check_read(self, view_name: str, success_msg: str, pk: int, field_value: bool = True):
        "Check permission, redirection, field updates, invalid cases."
        #       user1       user2
        # -----------       -----------  read
        #            <------              1
    ...
    def check_read_conversation(self, view_name:str, pk: int, field_value: bool = True):
        "Check redirection, field updates, invalid cases."
        #       user1       user2
        # -----------       -----------  read
        #           |<------              1
...
class FieldTest(BaseTest):
    ...
    def test_user_filter(self):
        ...
        from postman.fields import CommaSeparatedUserField
        def fct_none(u: 'AbstractUser'): return None
        f = CommaSeparatedUserField(user_filter=fct_none)
        self.assertEqual(frozenset(f.clean('foo, bar')), frozenset([self.user1, self.user2]))
        # no reason
        def fct_no_reason(u: 'AbstractUser'): return '' if u == self.user1 else None
        f = CommaSeparatedUserField(user_filter=fct_no_reason)
        self.assertRaises(ValidationError, f.clean, 'foo, bar')
        # with reason
        def fct_reason(u: 'AbstractUser'): return 'some reason' if u == self.user1 else None
        f = CommaSeparatedUserField(user_filter=fct_reason)
        self.assertRaises(ValidationError, f.clean, 'foo, bar')
...
class MessageManagerTest(BaseTest):
    ...
    def test(self):
        ...
        def pk(x: 'Message'): return x.pk
        def pk_cnt(x: 'Message'): return (x.pk, x.count)
        ...
        self.check_now(cast(datetime, m.read_at))
...
class MessageTest(BaseTest):
    ...
    """
    def check_parties(self, m: 'Message', s: 'AbstractUser | None' = None, r: 'AbstractUser | None' = None, email: str = ''):
    ...
    def test_moderation_from_pending(self):
        ...
        self.check_now(cast(datetime, m.moderation_date))
        self.check_now(cast(datetime, m.recipient_deleted_at))
        # pending -> accepted
        m = copy.copy(msg)
    ...
    def test_visitor(self):
        ...
        m.clean_for_visitor()
        self.check_status(m, sender_deleted_at=True)
        self.check_now(cast(datetime, m.sender_deleted_at))
        # as the recipient
    ...
    def test_update_parent(self):
        ...
        # a rejected reply
        Message.objects.create(subject='s', sender=self.user2, recipient=self.user1,
        ...
        self.check_status(cast(Message, r.parent), status=STATUS_ACCEPTED, thread=parent)
        ...
        self.assertEqual(cast(datetime, p.replied_at).timetuple(), r.sent_at.timetuple())  # mysql doesn't store microseconds
        ...
        self.check_status(p, status=STATUS_ACCEPTED, thread=parent, is_replied=True)
        self.assertEqual(cast(datetime, p.replied_at).timetuple(), r.sent_at.timetuple())

        # a reply is withdrawn and no other reply
        r = copy.deepcopy(reply)
        cast(Message, r.parent).replied_at = r.sent_at
        ...
        r.update_parent(STATUS_REJECTED)
        self.check_status(cast(Message, r.parent), status=STATUS_ACCEPTED, thread=parent, is_replied=True)
        ...
        p = Message.objects.get(pk=parent.pk)
        self.check_status(p, status=STATUS_ACCEPTED, thread=parent, is_replied=True)
        self.assertEqual(cast(datetime, p.replied_at).timetuple(), other_reply.sent_at.timetuple())
        # note: accepted -> pending, with no other suitable reply
        # is covered in the accepted -> rejected case

    def check_notification(self, m: 'Message', mail_number: int, email: str | None = None, is_auto_moderated: bool = True, notice_label: str | None = None):
    ...
    def test_notification_rejection_user_inactive(self):
        ...
        self.user1.is_active = False
        from postman.utils import notification
        args = (1, self.user1.email) if notification else (0, None)  # pinax.notifications (v5.0.3) doesn't filter on is_active
    ...
    def test_notification_acceptance_user_inactive(self):
        ...
        from postman.utils import notification
        args = (1, self.user2.email) if notification else (0, None)  # same as test_notification_rejection_user_inactive
    ...
    def check_auto_moderation(self, msg: 'Message', seq: 'Iterable[tuple[AutoModeratorsAlias, str | Literal[True] | None]]', default: str):
    ...
    def test_auto_moderation(self):
        ...
        def moderate_as_none(m: 'Message'):              return None
        def moderate_as_true(m: 'Message'):              return True
        def moderate_as_false(m: 'Message'):             return False
        def moderate_as_0(m: 'Message'):                 return 0
        def moderate_as_100(m: 'Message'):               return 100
        def moderate_as_50(m: 'Message'):                return 50
        def moderate_as_49_default_reason(m: 'Message'): return 49
        moderate_as_49_default_reason.default_reason = 'moderate_as_49 default_reason'  # type: ignore
        def moderate_as_49_with_reason(m: 'Message'):    return (49, 'moderate_as_49 with_reason')
        moderate_as_49_with_reason.default_reason = 'is not used'  # type: ignore
        def moderate_as_1(m: 'Message'):                 return (1, 'moderate_as_1')
        def moderate_as_1_no_reason(m: 'Message'):       return (1, ' ')
        def moderate_as_2(m: 'Message'):                 return (2, 'moderate_as_2')
        def moderate_as_98(m: 'Message'):                return 98
        moderate_as_98.default_reason = 'useless; never used'  # type: ignore
        def moderate_badly_as_negative(m: 'Message'):    return -1
        def moderate_badly_as_too_high(m: 'Message'):    return 101
        def moderate_as_0_with_reason(m: 'Message'):     return (0, 'moderate_as_0 with_reason')
        def invalid_moderator_1(m: 'Message'):           return (0, )
        def invalid_moderator_2(m: 'Message'):           return (0, 'reason', 'extra')

        for mod in [invalid_moderator_1, invalid_moderator_2]:
            m = copy.copy(msg)
            self.assertRaises(ValueError, m.auto_moderate, mod)

        seq: 'Iterable[tuple[AutoModeratorsAlias, str | Literal[True] | None]]' = (
            ...
            (moderate_as_50, True),
            # reasons
            (moderate_as_49_default_reason, moderate_as_49_default_reason.default_reason),  # type: ignore
...
class PendingMessageManagerTest(BaseTest):
    ...
    def test(self):
        def pk(x: 'Message'): return x.pk
        msg1 = self.create()
        self.create(moderation_status=STATUS_REJECTED)  # msg2
        self.create(moderation_status=STATUS_ACCEPTED)  # msg3
        msg4 = self.create()
        self._assertQuerySetEqual(PendingMessage.objects.all(), [msg4.pk, msg1.pk], transform=pk)
...
class FiltersTest(BaseTest):
    ...
    Test the filters.
    """
    def check_sub(self, x: str, y: str, value: str):
    ...
    def check_or_me(self, x: str, value: str, user: 'AbstractUser | None' = None, m: 'Message | None' = None):
    ...
    def check_compact_date(self, date: datetime, value: str, format: str = 'H:i,d b,d/m/y'):
...
class ContextProcessorsTest(BaseTest):
    ...
    """
    def _check_inbox(self, urlname: str, num_extra_queries: int, value: str = ''):
    ...
    def check_inbox_without_eval(self, num_extra_queries: int = 0):
        return self._check_inbox('no_context_processors', num_extra_queries)

    def check_inbox_with_eval(self, num_extra_queries: int = 0, value: str = ''):
...
class TagsTest(BaseTest):
    ...
    """
    def check_postman_unread(self, value: str, user: 'AbstractUser | AnonymousUser | None' = None, asvar: str = ''):
    ...
    def check_order_by(self, keyword: str, value_list: list[tuple[str, str]], context: str | None = None):
...
class UtilsTest(BaseTest):
    ...
    def check_email(self, message_template_name: str, recipient_list: 'Sequence[str]', object: 'Message', action: str, site: 'SiteAlias', parts: str | tuple[str, str] | None = None):
        mail.outbox = cast(list['EmailMessage'], [])
    ...
    def check_context_email(self, m: 'Message', action: str, setting: str | Callable[[dict[str, Any]], dict[str, Any]]):
        mail.outbox = cast(list['EmailMessage'], [])
        ...
        self.assertEqual(mail.outbox[0].subject, '{}_{}'.format(cast('AbstractUser', m.sender).username, action))
    ...
    def test_context_email(self):
        ...
        # a function
        self.check_context_email(m, action, lambda context: dict(context, extra_key='{}_{}'.format(cast('AbstractUser', m.sender).username, action)))

        # a path to a function
        self.check_context_email(m, action, 'postman.module_for_tests.context_email')

    def check_from_email(self, subject_template: str, message_template_name: str, recipient_list: 'Sequence[str]', object: 'Message', action: str, site: 'SiteAlias', from_email: str):
        mail.outbox = cast(list['EmailMessage'], [])
        email(subject_template, message_template_name, recipient_list, object, action, site)
    ...
    def test_from_email(self):
        ...
        def some_fct(context: Any): return custom_from_email
        settings.POSTMAN_FROM_EMAIL = some_fct
        self.reload_modules()
        self.check_from_email(subject_template, message_template_name, recipient_list, m, action, site, custom_from_email)

        # a path to a function
        custom_from_email = '{}_{}@domain.tld'.format(cast('AbstractUser', m.sender).username, action)
        settings.POSTMAN_FROM_EMAIL = 'postman.module_for_tests.from_email'
        self.reload_modules()
        self.check_from_email(subject_template, message_template_name, recipient_list, m, action, site, custom_from_email)

    def check_params_email(self, m: 'Message', action: str, site: 'SiteAlias', setting: str | Callable[[dict[str, Any]], dict[str, Any]], username: str, header: str):
        mail.outbox = cast(list['EmailMessage'], [])
    ...
    def test_params_email(self):
        ...
        # a path to a function
        self.check_params_email(m, action, site, 'postman.module_for_tests.params_email', cast('AbstractUser', m.sender).username, action)

    def check_notification_approval(
            self,
            m: 'Message',
            setting: Literal[False] | str | Callable[[Any, Any, Any], str | bool | None],
            mail_number: int,
            email: str | None = None
    ):
    ...
    def test_notification_approval(self):
        ...
        m = self.c12()
        from postman.utils import notification
        args = (1, self.user2.email) if notification else (0, None)
        # a constant
    ...
    def test_get_order_by(self):
        ...
        self.assertEqual(get_order_by({ORDER_BY_KEY: 'f'}), 'sender__{0}'.format(cast(type['AbstractUser'], get_user_model()).USERNAME_FIELD))
    ...
    def test_get_user_representation(self):
        ...
        self.assertEqual(get_user_representation(self.user1), "('foo',)")
        # a function
        def natural_key(u: 'AbstractBaseUser'): return u.natural_key()
...
class ApiTest(BaseTest):
    ...
    """
    def check_message(self, m: 'Message', subject: str = 's', body: str = 'b', recipient_username: str = 'bar'):
    ...
    def test_pm_write(self):
        ...
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', body='b')
        m = Message.objects.get()
        self.check_status(m, status=STATUS_ACCEPTED, moderation_date=True)
        self.check_now(cast(datetime, m.moderation_date))
        self.check_message(m)
    ...
    def test_pm_write_skip_notification(self):
        "Test the notification skipping."
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', skip_notification=True)
        self.assertEqual(len(mail.outbox), 0)

    def test_pm_write_auto_archive(self):
        "Test the auto_archive parameter."
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', auto_archive=True)
        m = Message.objects.get()
    ...
    def test_pm_write_auto_delete(self):
        ...
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', auto_delete=True)
        m = Message.objects.get()
    ...
    def test_pm_write_auto_moderators_accepted(self):
        ...
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', auto_moderators=lambda m: True)
        m = Message.objects.get()
        self.check_status(m, status=STATUS_ACCEPTED, moderation_date=True)
    ...
    def test_pm_write_auto_moderators_pending(self):
        ...
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', auto_moderators=[lambda m: None])
        m = Message.objects.get()
        self.check_status(m)
    ...
    def test_pm_write_auto_moderators_rejected(self):
        ...
        _ = pm_write(sender=self.user1, recipient=self.user2, subject='s', auto_moderators=(lambda m: False, ))
        m = Message.objects.get()
        self.check_status(m, status=STATUS_REJECTED, moderation_date=True, recipient_deleted_at=True)
        self.check_now(cast(datetime, m.moderation_date))
        self.check_now(cast(datetime, m.recipient_deleted_at))
...
class CommandTest(BaseTest):
    ...
    def test_cleanup_messages(self):
        ...
        m1 = self.create()
        _ = self.create(sender_deleted_at=good_date, recipient_deleted_at=good_date)  # the only good candidate
    ...
    def test_cleanup_conversations(self):
        ...
        _ = self.c21(parent=c1_m1, thread=c1_m1.thread, sender_deleted_at=good_date, recipient_deleted_at=good_date)
    ...
    def check_checkup(self, reasons: list[str]):
        count = len(reasons)
    ...
    def test_checkup_replied(self):
        delta = timedelta(minutes=1)
        _ = self.c12(replied_at=now() - delta)
        self.check_checkup([
    ...
    def test_checkup_reply(self):
        p = self.c12()
        _ = self.c21(parent=p)  # thread is missing
